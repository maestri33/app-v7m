    ...
    <Card>
      ...
      <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-2">
        {Object.entries(obj).map(([k, v]) => (
          <div key={k}>
            <dt className="text-xs uppercase tracking-wide text-muted-on-light">
              {k.replace(/_/g, " ")}
            </dt>
            <dd className="text-black break-words">{renderValue(v)}</dd>
          </div>
        ))}
      </dl>
    </Card>
  ...
  return (
    <Container narrow className="py-10">
      <Link
        ...
        className="text-sm text-muted-on-light hover:text-black underline"
      >
        ← Voltar pros leads
      </Link>

      {errorCode ? (
        <Card className="mt-6 text-muted-on-light">
          {notInHub
